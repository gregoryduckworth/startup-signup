  ...
  app.post("/api/waitlist", async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        console.log(
          `Confirmation email to ${validatedData.data.email}: ${
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!deleted) {
          return res
            .status(500)
            .json({ message: "Failed to delete waitlist entry" });
        }

        res
          .status(200)
          .json({ message: "Waitlist entry deleted successfully" });
      } catch (error) {
        console.error("Error deleting waitlist entry by email:", error);
